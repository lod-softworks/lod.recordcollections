...
{
    private static Dictionary<TKey, TValue> CloneDictionary(IDictionary<TKey, TValue> dictionary) =>
        dictionary.ToDictionary(kv => kv.Key, kv => RecordCloner.TryClone(kv.Value) ?? kv.Value);
